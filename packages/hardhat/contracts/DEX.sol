...
contract DEX {
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        if (token.transfer(msg.sender, tokenOutput) == false)
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        ...
        if (token.transferFrom(msg.sender, address(this), tokenInput) == false)
            revert DEX__TokenTransferFailed();
    ...
    function deposit() public payable returns (uint256 tokenDeposit) {
        ...
        if (
            token.transferFrom(msg.sender, address(this), tokenDeposit) == false
        ) revert DEX__TokenTransferFailed();
    ...
    function withdraw(
        ...
        if (token.transfer(msg.sender, token_amount) == false)
            revert DEX__TokenTransferFailed();
